...
=======
# # =============================================================================
# # Test evaluation — magnetic moment vector labels (6 cols)
# # =============================================================================
...
parser.add_argument("--test_voltage",  default="grid_calib_data.csv",
                    help="File voltage gốc (cùng file dùng lúc train2)")
parser.add_argument("--test_label",    default="Grid_points_coordinates.csv",
                    help="File label gốc (cùng file dùng lúc train2)")
parser.add_argument("--ckpt_dir", default="./ckpt_mvec")
parser.add_argument("--code_dir", default=".")
...
print("=" * 65)
print("  Test2 Evaluation  (held-out split từ train2)")
print("=" * 65)
print(f"  Voltage  : {args.test_voltage}")
print(f"  Label    : {args.test_label}")
...
ax2.set_ylabel("Position error (mm)", fontsize=12)
ax2.set_title(
    f"Position Error per Sample  [Held-out test split]\n"
    f"Mean = {mean_pos*1000:.2f} mm  |  RMSE = {rmse_pos*1000:.2f} mm  |  Max = {max_pos*1000:.2f} mm",
    fontsize=12)
ax2.legend(fontsize=10); ax2.grid(True, ls="--", alpha=0.5)
ax2.set_xlim(1, N); ax2.set_ylim(bottom=0); plt.tight_layout()
...
ax3.set_ylabel("Orientation error (°)", fontsize=12)
ax3.set_title(
    f"Orientation Error per Sample  [Held-out test split]\n"
...
# =============================================================================
# Test evaluation — magnetic moment vector labels (6 cols)
# Load test set từ split_info2.json được tạo bởi train_mvec.py
# =============================================================================
...
parser = argparse.ArgumentParser()
parser.add_argument("--test_voltage",  default="grid_calib_data.csv",
                    help="File voltage gốc đã dùng lúc train (để lấy test split)")
parser.add_argument("--test_label",    default="Grid_points_coordinates.csv",
                    help="File label gốc đã dùng lúc train (để lấy test split)")
parser.add_argument("--ckpt_dir", default="./ckpt_mvec")
parser.add_argument("--code_dir", default=".")
parser.add_argument("--out",      default="test_result.png")
args = parser.parse_args()
...
def _read(path):
    # Dùng header=0 giống train_mvec.py
    df = pd.read_csv(path, header=0)
    return df.apply(pd.to_numeric, errors="coerce").dropna().reset_index(drop=True)
...
# =============================================================================
# Load data — chỉ lấy phần test_idx từ split_info2.json
# =============================================================================
...
assert "test" in split_info, (
    "split_info2.json không có key 'test'. "
    "Hãy chạy lại train_mvec.py với --test_ratio > 0."
)
test_idx = np.array(split_info["test"])
print(f"  test_idx size : {len(test_idx)}")

# Đọc toàn bộ data gốc rồi slice theo test_idx
volt_df  = _read(args.voltage)
label_df = _read(args.label)

assert volt_df.shape[1]  == 64, f"Voltage cần 64 cols, có {volt_df.shape[1]}"
assert label_df.shape[1] == 6,  f"Label cần 6 cols [x,y,z,mx,my,mz], có {label_df.shape[1]}"

voltages = volt_df.values.astype(np.float32)
labels   = label_df.values.astype(np.float32)
Nfull    = min(len(voltages), len(labels))
voltages = voltages[:Nfull][test_idx]   # slice test
labels   = labels[:Nfull][test_idx]     # slice test
N        = len(test_idx)
print(f"  Test samples  : {N}")

# Load scalers (fit trên train, không fit lại)
scaler_path = os.path.join(args.ckpt_dir, "scalers.pkl")
print(f"  Loading scalers from {scaler_path} ...")
with open(scaler_path, "rb") as f:
    scalers = pickle.load(f)
...
assert scalers.get("label_format") == "mvec", (
    f"Expected label_format='mvec' in scalers.pkl, got {scalers.get('label_format')!r}. "
    "Dùng test_mvec.py với checkpoint từ train_mvec.py."
)
volt_scaler  = scalers["volt"]
label_scaler = scalers["label"]
...
>>>>>>> a7a7e1b0ca5fb83b809a2d73c8954ca98e8e71fa
=======
plt.show()
>>>>>>> Stashed changes
