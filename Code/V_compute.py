...
def compute_Bz(roi_xyz, sensor_pos, m_vecs):
    ...
    Return:
        Bz : (N,64)
    """
    r_vec = sensor_pos[None, :, :] - roi_xyz[:, None, :]  # (N,64,3)
    ...
    return B_vec[:, :, 2]  # chỉ lấy Bz


# ─── Load calibration ─────────────────────────────────────────────────────────
print("Loading calibration...")

calib_df = pd.read_csv(BASE_DIR / "Calibration_PARAM.csv")
...
print(f"Number of sensors : {len(sensor_pos)}")
...
m_vecs = coord_df[["mx", "my", "mz"]].values

print(f"Number of positions : {len(roi_xyz)}")
...
V = offset_arr[None, :] + Bz * gain_arr[None, :]

print("Voltage shape:", V.shape)

# ─── Save CSV ─────────────────────────────────────────────────────────────────
column_names = [f"sensor {i}" for i in range(1, 65)]
...
print(f"Output shape: {voltage_df.shape}")
