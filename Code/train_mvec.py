...
def get_config():
    ...
    p.add_argument("--lambda_ori",     type=float, default=1.0)
...
def main():
        ...
        if os.path.exists(log_file):
            with open(log_file) as f:
                try:
                    ...
                    train_physics_history = [x["train_physics"] for x in history]
                    val_physics_history   = [x["val_physics"]   for x in history]
                except: pass
        start_epoch += 1
    ...
    no_improve = 0
    # train_ang / val_ang log cosine orientation loss (loss_ori)
    hdr = (f"{'Epoch':>6}  {'Train':>9}  {'Val':>9}  "
           f"{'Val_xyz':>9}  {'Val_ori':>9}  {'Val_phys':>9}  {'LR':>8}  {'Time':>7}")
    print(hdr)
    print("-" * len(hdr))
    ...
    for epoch in range(start_epoch, cfg.num_epochs + 1):
        ...
        if val_loss < best_val:
            ...
            print(f"          >> Best saved  val={best_val:.6f} "
                  f"(xyz={val_xyz:.5f}  ori={val_ang:.5f}  phys={val_physics:.5f})",
                  flush=True)
        else:
            no_improve += 1
...
# # =============================================================================
# # SPLIT TRAIN/VAL/TEST  — magnetic moment vector labels (6 cols)
# # =============================================================================
# import os, sys, json, pickle, argparse, time
# import uuid
# import numpy as np
...
#     p.add_argument("--test_ratio",     type=float, default=0.007,       # THÊM MỚI
#                    help="Ti le test held-out (default=0.15)")
#     p.add_argument("--batch_size",     type=int,   default=64)
#     p.add_argument("--num_epochs",     type=int,   default=200)
#     p.add_argument("--lr",             type=float, default=1e-3)
#     p.add_argument("--weight_decay",   type=float, default=0.00456)
...
#                    scaler_file, seed=42):
#     """
#     Split train / val / test từ 1 tập duy nhất.
#     test_idx được lưu vào split_info2.json để test_mvec.py dùng lại.
#     Trả về (train_ds, val_ds, n_train, n_val).
#     """
...
#     # THÊM MỚI: Lưu cả 3 index vào split_info2.json để test_mvec.py dùng lại
#     split_path = os.path.join(os.path.dirname(scaler_file), "split_info2.json")
#     if not os.path.exists(split_path):
#         with open(split_path, "w") as f:
#             json.dump({
...
#                 "seed":  seed,
#                 "mode":  "random_split",                               # THÊM MỚI: đánh dấu mode
#             }, f)
#         print(f"  Split info saved -> {split_path}")
...
#     print("\n" + "=" * 65)
#     print("  Model Training (m_vec labels, train / val / test split)")
#     print("=" * 65)
#     gpu_name = torch.cuda.get_device_name(0) if device.type == "cuda" else "CPU"
#     print(f"  Device      : {device} ({gpu_name})")
#     print(f"  Voltage     : {cfg.voltage}")
...
#     print(f"  Test        : held-out từ cùng tập (run test_mvec.py --mode split)")
#     print(f"  Epochs      : {cfg.num_epochs}  |  Batch: {cfg.batch_size}  |  LR: {cfg.lr}")
#     print(f"  Optimizer   : AdamW  weight_decay={cfg.weight_decay}")
#     print(f"  Scheduler   : LinearLR warmup ({cfg.warmup_epochs} ep) -> CosineAnnealingLR")
#     print(f"  Loss        : HuberPoseLossMVec  lambda_ori={cfg.lambda_ori}"
